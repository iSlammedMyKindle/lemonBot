...
client.on('messageCreate', async msg => {
    ...
    // Bots can wind up getting to a message before lemonbot, so if that happens, just throw an exception and don't attempt to respond.
    try {
        //This is a bug - we need to wait for discord.js to fix the thing (current version: 14.3.0)
        msg = await msg.fetch();
    }
    catch (e) {
        // Display the message ID and the guild ID, that can at least help figure out a timestamp as to when this happens
        console.error('A bot probably deleted a message before I could read it... quietly failing\n', e);
        console.error('GuildId:', msg.guildId, 'Message Id:', msg.id);
        return;
    }
